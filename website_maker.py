...
def build_html(all_data: dict) -> str:
	...
	cpu_temp_min_c = system_info.get("cpu_temp_min_c")
	cpu_temp_max_c = system_info.get("cpu_temp_max_c")
	cpu_temp_ts = system_info.get("cpu_temp_timestamp")

	if cpu_temp_c is not None:
		cpu_temp_text = f"{cpu_temp_c:.1f} °C"
	else:
		cpu_temp_text = "not available"

	if cpu_temp_min_c is not None and cpu_temp_max_c is not None:
		cpu_minmax_text = f"{cpu_temp_min_c:.1f}–{cpu_temp_max_c:.1f} °C"
	else:
		cpu_minmax_text = "not available"

	cpu_ts_text = cpu_temp_ts or "—"
...
"""
		...
		<div class=\"card\">
			<h2>Raspberry Pi 5 - CPU temperature</h2>
			<p>Current CPU temperature: """ + cpu_temp_text + """</p>
			<p>Session min/max: """ + cpu_minmax_text + """</p>
			<p>Last updated: """ + cpu_ts_text + """</p>
		</div>
